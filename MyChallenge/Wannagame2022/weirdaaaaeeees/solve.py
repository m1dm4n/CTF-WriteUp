...
import os
# io = process('./weirdaaaaeeees')
io = remote('45.122.249.68', 20021)
...
def mixColumnInv(column):
    ...
    column[0] = galoisMult(temp[0], 0xE) ^ galoisMult(
        temp[3], 0x9) ^ galoisMult(temp[2], 0xD) ^ galoisMult(temp[1], 0xB)
    column[1] = galoisMult(temp[1], 0xE) ^ galoisMult(
        temp[0], 0x9) ^ galoisMult(temp[3], 0xD) ^ galoisMult(temp[2], 0xB)
    column[2] = galoisMult(temp[2], 0xE) ^ galoisMult(
        temp[1], 0x9) ^ galoisMult(temp[0], 0xD) ^ galoisMult(temp[3], 0xB)
    column[3] = galoisMult(temp[3], 0xE) ^ galoisMult(
        temp[2], 0x9) ^ galoisMult(temp[1], 0xD) ^ galoisMult(temp[0], 0xB)
...
def round2master(rk):
    ...
    while j < Nb*(Nr+1):
        if (j % Nk) == 0:
            w[j][0] = w[j-Nk][0] ^ sbox[w[j-1][1]
                                        ^ w[j-2][1]] ^ Rcon[Nr - j//Nk][0]
            for i in range(1, 4):
                w[j][i] = w[j-Nk][i] ^ sbox[w[j-1]
                                            [(i+1) % 4] ^ w[j-2][(i+1) % 4]]
        else:
...
key1 = os.urandom(8) + bytes.fromhex("E0FEE0FEF1FEF1FE") + bytes.fromhex("FEE0FEE0FEF1FEF1")
io.sendlineafter(b'Key: ', key1.hex().encode())
# io.interactive()
# ciphertexts = eval(open('log.txt', 'r').read())

ciphertexts = []
for i in range(256):
    io.recvuntil(b':\n')
    ciphertexts.append(io.recvline().strip().decode())
    io.sendlineafter(b':\n', b'1')

print(len(ciphertexts))
ciphertexts = [[c for c in bytes.fromhex(x)] for x in ciphertexts]
